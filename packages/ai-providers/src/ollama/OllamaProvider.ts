...
/**
 * Ollama provider for natural language timeline commands using local LLMs
 * Uses Ollama's OpenAI-compatible API endpoint
 */
export class OllamaProvider implements AIProvider {
  id = "ollama";
  name = "Ollama";
  description = "Local LLM for natural language timeline control";
  ...
  isConfigured(): boolean {
    // Ollama doesn't require API key, just needs the server to be running
    return true;
  }
...
Or if clarification needed:
{
  ...
  "clarification": "Which clip do you want to trim?"
      ...
      console.error("Ollama parseCommand error:", error);
      return this.fallbackParse(instruction, context);
    }
  }
